...
def handle_callback_query_term(call):
    ...
    button_moroz = types.InlineKeyboardButton(text="Морозец", callback_data="a89")
    button_back = types.InlineKeyboardButton(text="Назад⬇️", callback_data="back")
    markup.add(button_pt, button_r, button_s, button_v, button_k, button_f, button_vi, button_moroz, button_back)
    bot.edit_message_text(chat_id=chat_id, message_id=call.message.message_id,
                          text="Хорошо! Теперь выберете  артефакт:", reply_markup=markup)
...
@bot.callback_query_handler(
    func=lambda call: call.data in ["a38", "a39", "a41", "a42", "a43", "a44", "a45", "a46", "a47", "a48", "a49", "a50", "a51", "a52", "a53", "a54", "a55", "a56", "a57", "a58", "a59", "60", "a61", "a62", "a63", "a64", "a65", "a89"])
